...
`mode` values: `rag`, `llm-wiki`. `tenant_scope` is `default` for blank or `default` tenants, `custom` otherwise.
...
- ~~Add tenant-scoped retrieval boundary.~~ Done: uploads persist `tenantId`, RAG/Wiki/Bot retrieval filters hydrated chunks by tenant, and missing tenant values default to `default`.
...
Rules are scoped with the Micrometer `application="campus-rag-qa"` tag so they can share a Prometheus server with other services.
