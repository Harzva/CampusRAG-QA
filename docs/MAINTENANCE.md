...
| Operations | Check health, 5xx rate, p95 latency, JVM memory, and disk usage. |
| Retrieval quality | Re-run a small campus QA evaluation set before changing chunking or prompts. |
...
| p95 latency | More than 8 seconds for 10 minutes | Check model latency and vector store latency. |
| JVM memory | More than 85% for 10 minutes | Inspect document ingestion and chunk size. |
