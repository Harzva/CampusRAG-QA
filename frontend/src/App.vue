...
const productName = 'CampusRAG-QA';
const productSubtitle = 'Campus knowledge QA baseline with switchable RAG retrieval and Wiki memory modes.';
...
const readinessSubtitle = 'Gateway, observability, and deployment surfaces prepared for production review.';
...
const readinessSignals = [
  ...
  { label: 'Security', value: 'HMAC + timestamp', detail: 'Replay-resistant Bot callback contract' },
  { label: 'Deploy', value: 'Compose + K8s', detail: 'Local stack and cluster template included' },
];

const botChannels = ['Feishu Bot', 'DingTalk Bot', 'WeChat Bot', 'RAG', 'Wiki'];
...
function applyRoutePreview() {
  ...
  if (demo === 'conversation') {
    messages.value = [
      { id: ++messageCounter, role: 'User', content: 'How do students apply for campus card replacement?' },
      {
        id: ++messageCounter,
        role: 'Assistant',
        content: `${currentMode.value.label} checked retrieved policy chunks and returned a grounded answer with missing-evidence guardrails.`,
      },
    ];
  }
  if (demo === 'upload') {
    uploadStatus.value = `Indexed handbook.md for ${currentMode.value.label}.`;
  }
}
